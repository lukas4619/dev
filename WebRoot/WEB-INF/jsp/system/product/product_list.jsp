...
<body class="no-skin">
				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="product/list.do" method="post" name="Form"
								id="Form">
								<table style="margin-top:5px;">
									<tr>
										...
										<td style="vertical-align:top;padding-left:2px;"><select
											class="chosen-select form-control" data="${pd.proState }"
											name="proState" id="proState" data-placeholder="请选择"
											style="vertical-align:top;width: 120px;">
												<option value="-1">产品状态</option>
												<!-- 开始循环 -->
												<c:choose>
													<c:when test="${not empty proStateList}">
														<c:forEach items="${proStateList}" var="s" varStatus="vss">
								...
								<table id="simple-table"
									...
									<thead>
										<tr>
											...
											<th class="center">产品封面图片</th>
											<th class="center">产品预订次数</th>
											<th class="center">产品成功消费次数</th>
											<th class="center">产品创建时间</th>
											<th class="center">产品编辑时间</th>
											...
											<c:when test="${not empty varList}">
												<c:if test="${QX.cha == 1 }">
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															<td class='center' style="vertical-align: middle;"><label class="pos-rel"><input
																	type='checkbox' name='ids' value="${var.productId}"
															...
															<img src="<%=basePath%>uploadFiles/uploadImgs/${var.proImg}"  width="100"/>
															</td>
															<td class='center' style="vertical-align: middle;">${var.proReservationNum}</td>
															<td class='center' style="vertical-align: middle;">${var.proconsumeNum}</td>
															<td class='center' style="vertical-align: middle;">
															...
															<td class='center' style="vertical-align: middle;">
															<label>
        													<input name="switch-field-1" onclick="editByState('${var.productId}','${var.proState}');" class="ace ace-switch ace-switch-3" <c:if test="${var.proState==15}"> checked="checked" </c:if>  type="checkbox">	<span class="lbl"></span>
    														</label>
															</td>
															<td class='center' style="vertical-align: middle;">${var.adminId}</td>
															<td class="center" style="vertical-align: middle;"><c:if
																	test="${QX.edit != 1 && QX.del != 1 }">
																	<span
																		class="label label-large label-grey arrowed-in-right arrowed-in"><i
																		class="ace-icon fa fa-lock" title="无权限"></i>
																	</span>
																</c:if>
																<div class="hidden-sm hidden-xs btn-group">
																	<c:if test="${QX.edit == 1 }">
																		<a class="btn btn-xs btn-success" title="编辑"
																			onclick="edit('${var.productId}');"> <i
																			class="ace-icon fa fa-pencil-square-o bigger-120"
																			title="编辑"></i> </a>
																	</c:if>
																	<c:if test="${QX.del == 1 }">
																		<a class="btn btn-xs btn-danger"
																			onclick="del('${var.productId}');"> <i
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			if($("#proState").attr('data')!='' && $("#proState").attr('data') !=-1){
		...
		function add(){
			 top.jzts();
			 window.location.href="<%=basePath%>product/goAdd.do";
		}
		...
		//修改状态
		function editByState(Id,proState){
			if(proState==15){
				proState=16
			}else{
				proState=15;
			}
			var url = "<%=basePath%>product/editByState.do?PRODUCTID="+Id+"&PROSTATE="+proState+"&tm="+new Date().getTime();
			$.get(url,function(data){
				//nextPage(${page.currentPage});
			});
		}
		...
		function edit(Id){
			 top.jzts();
			 window.location.href="<%=basePath%>product/goEdit.do?PRODUCTID="+Id;
		}
