...
public class frontMemberController extends BaseController {
	...
	public Object sendPhoneCode() throws Exception {
		...
		if(pd.getInt("sendType")==2) sendType = 2;//忘记密码;
		map = TaoBaoDaYu.sendMsgByAutoCode(phone, Code, sendType);// 调用阿里大于短信平台发送验证码
	...
	public Object codeVerify() throws Exception {
		...
		p.put("CONTENTS", getMemberName() + "[认证成为作者并绑定手机号码" + phone + "]");
		p.put("CREATEDATE", new Date());
		p.put("LOGIP", PublicUtil.getIp());
		p.put("MEMBERID", getMemberId());
		// 记录用户操作信息
		memberService.editPhoneNumber(pd, p);
