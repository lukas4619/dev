...
public class OrderController extends BaseController {
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var2", varOList.get(i).getString("BILLNO"));	    //2
			vpd.put("var3", varOList.get(i).get("ORDERSTATUSID").toString());	//3
			vpd.put("var4", varOList.get(i).get("PAYMENTSTATUSID").toString());	//4
			vpd.put("var5", varOList.get(i).get("PLUCOUNT").toString());	//5
			vpd.put("var6", varOList.get(i).getString("TOTALPRICE"));	    //6
			vpd.put("var7", varOList.get(i).getString("PAYMENTDATE"));	    //7
			vpd.put("var8", Convert.dateToStr(Convert.strToDate(varOList.get(i).get("CREATEDATE").toString(), new Date()),"yyyy-MM-dd HH:mm:ss", "1900-01-01 00:00:00"));	    //8
			vpd.put("var9", varOList.get(i).get("STORENAME").toString());	//9
			vpd.put("var10", varOList.get(i).get("SHELVESNAME").toString());	//10
			vpd.put("var11", varOList.get(i).get("PLACENAME").toString());	//11
			vpd.put("var12", varOList.get(i).getString("WECHATNAME"));	    //12
			varList.add(vpd);
		}
